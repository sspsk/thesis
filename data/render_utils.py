#code adapted from https://github.com/nkolot/ProHMR/blob/master/prohmr/utils/renderer.py
...
class Renderer:
    ...
    def __call__(self,
        ...
        if full_frame:
            ...
            image = image.clone() * torch.tensor(self.IMAGE_STD, device=image.device).reshape(3,1,1)
            image = image + torch.tensor(self.IMAGE_MEAN, device=image.device).reshape(3,1,1)
